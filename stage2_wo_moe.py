...
def generate_run_name(config):
    ...
    # Mark run as MoE-free for clarity
    run_name = (
        f'{seed}_VQ{num_embed}_{config["data"]["universe"]}_woMOE_'
...
def train(config, train_loader,
    ...
    if wandb_run:
        ...
        project_name = config['train']['project_name']+'_'+config['data']['universe']+'_stage2_wo_moe'
    ...
    # wo_moe 폴더 생성 및 저장
    wo_moe_dir = os.path.join(get_root_dir(), config['train']['save_res'], 'wo_moe')
    os.makedirs(wo_moe_dir, exist_ok=True)
    
    output_path = os.path.join(wo_moe_dir, f"{run_name}_best.pkl")
    pred_df.to_pickle(output_path)

    output_csv_path = os.path.join(wo_moe_dir, f"{run_name}_metric.csv")
...
if __name__ == "__main__":
    from utils import load_args, load_yaml_param_settings
    args = load_args()
