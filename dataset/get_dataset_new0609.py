...
def load_args():
    ...
    parser.add_argument('--universe', type=str, help="Market universe (csi300 or sp500)",
                        default="csi300")
...
if __name__ == "__main__":
    ...
    factor_mat.columns = [f"JKP_{c}" for c in factor_mat.columns]
    ...
    # 1~10 거래일 뒤 수익률을 계산
    horizons = list(range(0, 10))  # [0, 1, …, 9]
    ...
    with open(f"./dataset/data/{region}/{args.universe}_{seq_len}_dl_train.pkl", "wb") as f:
        pickle.dump(dl_train, f)
    with open(f"./dataset/data/{region}/{args.universe}_{seq_len}_dl_valid.pkl", "wb") as f:
        pickle.dump(dl_valid, f)
    with open(f"./dataset/data/{region}/{args.universe}_{seq_len}_dl_test.pkl", "wb") as f:
        pickle.dump(dl_test, f)
    with open(f"./dataset/data/{region}/{args.universe}_{seq_len}_dl_dataset.pkl", "wb") as f:
